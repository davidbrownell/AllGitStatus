...
class TestGetRepositoryData:
    ...
    def test_Complete(self, tmp_path) -> None:
        ...
        assert "commit 4" in repo_data.local_changes[0], repo_data.local_changes[0]
        assert repo_data.local_changes[1].startswith("commit "), repo_data.local_changes[1]
        assert "commit 3" in repo_data.local_changes[1], repo_data.local_changes[1]

        assert len(repo_data.remote_changes) == 2
        assert repo_data.remote_changes[0].startswith("commit "), repo_data.remote_changes[0]
        assert "commit 2" in repo_data.remote_changes[0], repo_data.remote_changes[0]
        assert repo_data.remote_changes[1].startswith("commit "), repo_data.remote_changes[1]
        assert "commit 1" in repo_data.remote_changes[1], repo_data.remote_changes[1]


# BugBug: test_GetRepositoryData_NoRemote
# BugBug: test_GetRepositoryData_NoLocalChanges
# BugBug: test_GetRepositoryData_NoRemoteChanges
