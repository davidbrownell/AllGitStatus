...
class MainApp(App):
    ...
    def _OnRepositorySelectionChanged(self, *, repopulate_changes: bool = True) -> None:
        ...
        if data_item.local_changes:
            self._local_log.write(Group(*[Panel(change) for change in reversed(data_item.local_changes)]))

        if data_item.remote_changes:
            self._remote_log.write(Group(*[Panel(change) for change in reversed(data_item.remote_changes)]))
    ...
    def _ExecuteGitCommand(self, command: str) -> None:
        ...
        async def Execute() -> None:
            ...
            self._ResetRepository(data_item.path, self._data_table.cursor_row)
